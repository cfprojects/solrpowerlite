<cfcomponent>
	...
	<cfquery name='getConfig' datasource='#request.dsn#'>
		SELECT	*
		FROM	config 
	</cfquery>
	<!---
		DirectoryWatcher.cfc 
		...
		*     http://www.apache.org/licenses/LICENSE-2.0		 
		...
		@notes Incorporates SoldColdFusion client code for communicating with Solr server
		 --->
	...
	<cffunction name="init" access="public" output="false" returntype="DirectoryWatcher">
	...
	<cffunction name='onAdd' output='no'> 
	    ...
	    <cfset local.data = CFEvent.data /> 
	    <cflog file='solrPowerLite_DirWatcher' application='No' text='ACTION: #local.data.type#; FILE: #local.data.filename#; TIME: #timeFormat(local.data.lastmodified)#'/> 

		<cfindex type='file' action='update' collection='#getConfig.collectionName#' key='#local.data.filename#' urlpath='#getConfig.folderAlias#' />
	...
	<cffunction name='onDelete' output='no'> 
		<cfargument name='CFEvent' type='struct' required='yes' /> 
		<cfset local.data = CFEvent.data /> 
		<cflog file='solrPowerLite_DirWatcher' application='No' text='ACTION: #local.data.type#; FILE: #local.data.filename#'/> 
		<!---<cfindex action='delete' collection='#getConfig.collectionName#' key='#local.data.filename#'/>--->
    	<cfset local.params = arrayNew(1) />
		<cfset local.params[1][1] = "fl" />
		<cfset local.params[1][2] = "uid" />
		<cfset init("http://localhost", "8983", "/solr/solrpowerlite")/>
		<cfset local.result = xmlparse(search(q='key:#solrFormat(local.data.filename)#',start=0,limit=1,params=local.params))/>
		<cfset local.uid = xmlsearch(local.result,"/response/result//doc[1]/str[@name='uid']")[1].XmlText/>
		
		<cfset deleteById(id=uid)/>
		<cfset commit()/>
	</cffunction>
